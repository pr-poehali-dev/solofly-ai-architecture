...
export default function LandingHero({ onNavigate, onOpenDemo }: LandingHeroProps) {
  ...
  // requestAnimationFrame с throttle до ~20fps на мобильных, ~30fps на десктопе
  useEffect(() => {
    if (isMobile) return; // на мобильных canvas не рисуем — экономим батарею
    ...
    // Размеры вычисляем один раз при монтировании и при ресайзе
    const updateSize = () => {
    ...
    const draw = (now: number) => {
      ...
      // Статичные узлы + 3 анимированных (было 8)
      const nodes = [
        { x: W * 0.12, y: H * 0.25 },
        { x: W * 0.30, y: H * 0.65 },
        { x: W * 0.52, y: H * 0.20 },
      ...
      // Батч все линии одним путём для снижения вызовов stroke
      const DIST_SQ = 300 * 300; // сравниваем квадраты — без sqrt
      ctx.lineWidth = 0.6;
