...
def handle_callback(event: dict, origin: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Создаём сессию совместимую с основным auth (таблица sessions + sf_token)
            session_token = secrets.token_urlsafe(32)
            session_expires = (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()

            cur.execute(
                f"""INSERT INTO {S}sessions (user_id, token, expires_at, created_at)
                    VALUES (%s, %s, %s, %s)""",
                (user_id, session_token, session_expires, now)
            )

            # Также обновляем last_login для нового пользователя
            cur.execute(
                f"UPDATE {S}users SET last_login = %s WHERE id = %s",
                (now, user_id)
            )
